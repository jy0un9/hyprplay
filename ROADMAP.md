...
11. **README / AppStream screenshot**  
    `docs/assets/screenshot.png` is referenced but missing (only `.gitkeep`). Capture a real library + now-playing shot for store and README.
...
1. Broader format support (MP3/AAC/M4A) — or document FLAC/Opus-only as a permanent product choice  
2. Richer DB schema: genre, year, album-artist, and browse/filter by them  
3. Cover-art fetch that does not require a full Discogs token flow  
...
9. File associations / `MimeType` in the `.desktop` entry  
